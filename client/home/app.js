...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (category) {
    ...
    else if (pageTitle.includes("bellek") || pageTitle.includes("ram"))
      category = "RAM";
    else if (pageTitle.includes("depolama") || pageTitle.includes("ssd"))
      category = "SSD";
  ...
  const isHomePage =
    ...
    (!category && !window.location.pathname.includes("category.html"));
  ...
  try {
    ...
    if (response.ok) {
      ...
      displayProducts.forEach((product) => {
        ...
        const badge =
          product.Stock <= 0 ? '<span class="badge">Tükendi</span>' : "";
