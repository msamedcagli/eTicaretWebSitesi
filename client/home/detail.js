document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (priceFromUrl)
    document.getElementById("detailPrice").innerText = priceFromUrl;
  if (catFromUrl)
    document.getElementById("detailCategory").innerText = catFromUrl;
  if (titleFromUrl)
    document.getElementById("breadcrumbTitle").innerText = titleFromUrl;

  if (imgFromUrl) {
    const detailImg = document.getElementById("detailImg");
    // Görsel yolu yerel bir assets yoluysa başına ../ ekle
    detailImg.src = imgFromUrl.startsWith("/assets")
      ? `..${imgFromUrl}`
      : imgFromUrl;
  }
  ...
  if (favBtn) {
    ...
    favBtn.addEventListener("click", async () => {
      const kullaniciVerisi =
        getCookie("userSession") || localStorage.getItem("kullaniciBilgileri");
      if (!kullaniciVerisi) {
